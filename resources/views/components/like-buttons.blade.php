<div class="flex">
    <form action="{{ route('tweet.like', $tweet) }}" method="POST">
        ...
        <button class="flex items-center {{ $tweet->isLikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }} mr-4">  
    ...
    <form action="{{ route('tweet.dislike', $tweet) }}" method="POST">
        ...
        <button class="flex items-center {{ $tweet->isDislikedBy(current_user()) ? 'text-blue-500' : 'text-gray-500' }}">
            <svg viewBox="0 0 20 20" class="mr-1 w-4">
                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g class="fill-current">
